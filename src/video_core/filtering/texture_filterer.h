...
void filterTexture(Pica::Texture::TextureInfo tex_info, unsigned int* fromBuffer,
                   unsigned int* toBuffer);

// Returns the scaled texture size (width * height) of this texture
int getScaledTextureSize(Pica::TexturingRegs::TextureFormat format, int width, int height);
}
